...
We also run the following code in our *Console* window in order to create an [.Renviron]{.monospace} file and path to the [Twitter authentication token]{.monospace} in your RStudio Server account's user directory and then open the file in RStudio for editing later on.
...
The official method for accessing Twitter data is through their API (**application programming interface**), which is only available if you have an *authentication token*.
...
Navigate to https://apps.twitter.com and click [Create New App]{.monospace}.
...
If you need help choosing the Name, Description, and Website (which aren't important in this case), just use the following:

*   **Name:** [rtweet_tokens]{.monospace}

*   **Description:** CDS 102 lab

*   **Website:** <http://summer18.cds101.com>
...
After creating the app, click the tab labeled [Keys and Access Tokens]{.monospace} to retrieve your consumer (api) and secret keys.
...
You will now need to copy and paste the codes for the [Consumer Key]{.monospace}, [Consumer Secret]{.monospace}, [Access Token]{.monospace}, and [Access Token Secret]{.monospace} into the [.Renviron]{.monospace} file that we opened at the very end of the previous section.
Copy each of these codes, match it to one of the four keywords in the [.Renviron]{.monospace} file, and paste your copied code in the space immediately to the right of the equals `=` sign.
Then, in the RStudio menubar, click [Session]{.monospace}, then click [Restart R]{.monospace}
...
Our example account will be [`@CSS_GMU`][css-gmu-twitter-account], which is the official Twitter account of the Computational Social Science program in George Mason's Computational & Data Sciences Department.

We begin by fetching the basic account information for [`@CSS_GMU`][css-gmu-twitter-account], which we will then save to disk for offline access, and then explore the variables returned to us.
...
lookup_users("CSS_GMU") %>%
  write_rds(path = "user_css_gmu", ext = "rds"))
```
...
Now let's take a look at the information we just gathered about [`@CSS_GMU`][css-gmu-twitter-account].
...
@.  Let's focus on the following variables associated with the [`@CSS_GMU`][css-gmu-twitter-account] account:
...
@.  We need to fetch the friends and followers of the [`@CSS_GMU`][css-gmu-twitter-account] account using the `get_followers()` and `get_friends()` functions from [rtweet]{.monospace} as follows:
...
One of the questions we can ask of our data is if there is a relationship between how often [`@CSS_GMU`][css-gmu-twitter-account]'s followers tweet and the number of followers they have.
...
@.  Use the following command to get an answer to these questions:
    ...
    Also, what does this graph tell us about the accounts that follow [`@CSS_GMU`][css-gmu-twitter-account], and by consequence, what does all this say about [`@CSS_GMU`][css-gmu-twitter-account]?

The ages of the accounts for [`@CSS_GMU`][css-gmu-twitter-account]'s friends and followers could contain meaningful patterns and be of interest to us.
For example, it may be that the patterns in the account ages for [`@CSS_GMU`][css-gmu-twitter-account] look different from accounts not associated with a University.
    
@.  Use the following command to create a visualization that looks at the account creation date for followers of [`@CSS_GMU`][css-gmu-twitter-account]:
    ...
    Describe the distribution of ages of the accounts that follow [`@CSS_GMU`][css-gmu-twitter-account], and what can we infer about the account [`@CSS_GMU`][css-gmu-twitter-account] itself?
...
For example, to request all the tweets by the [`@CSS_GMU`][css-gmu-twitter-account] account we use the following command:
...
@.  Create a timeline plot of [`@CSS_GMU`][css-gmu-twitter-account]'s monthly tweeting frequency since the account was first created using the following code:
...
*   If you want to try and do some basic analysis on the actual text of the tweets, you're welcome to give it a try, although this is in no way required.
    The [[tidytext]{.monospace} R package][tidytext-book] is a good place to start if you want to try this out.
:::
